...
void
Context_Wrapper::_handle_ping_event(int fd, short event, void *opaque)
{
...
Context_Wrapper::_handle_sigint_event(int signo, short event, void *opaque)
{
	...
	utp_close(context_w->socket_tmp[0]);
}
	    ...
	    *salocal_p, int write_size, bool is_listener)
{
	...
	utp_context_set_option(this->context_u, UTP_LOG_NORMAL, 1);
	utp_context_set_option(this->context_u, UTP_LOG_MTU, 1);
	utp_context_set_option(this->context_u, UTP_LOG_DEBUG, 1);
...
//	_libevent_write = _handle_write_event;
	...
	if ((this->ev_timeo = event_new(this->ev_base, this->fd, EV_PERSIST,
            this->_handle_ping_event, this)) == NULL) {
                err(1, "event_new() failed");
                delete this;
        }

        memset(&tv, 0, sizeof(tv));
        tv.tv_usec = PING_INTERVAL;
        if (event_add(this->ev_timeo, &tv) < 0) {
                err(1, "event_add() failed");
                delete this;
        }
...
Context_Wrapper::_utp_write(void)
{
	...
	if (this->is_writable) {
		...
		if (this->socket_tmp[0] != NULL)
			result = utp_write(this->socket_tmp[0], (void *)  &msg,
			    sizeof(msg_t)); /*XXX*/
	}
...
Context_Wrapper::_utp_read(utp_callback_arguments *args)
{
	...
	ssize_t len, left;
	struct timeval *start_time_p, interval, end_time;
	msg_t *msg;
...
uint64
Context_Wrapper::_utp_log(utp_callback_arguments *args)
{
	debug("utp_log: %s\n", args->buf);
//	connection->disable_sigint_handler(); /* TODO: fix it */
...
//		/* TODO: fix it */ connection->disable_sigint_handler();
	...
	case UTP_STATE_DESTROYING:
		...
		if (context_stats) {
			debug("\n           Bucket size:    <23");
			debug("    <373    <723    <1400    >1400\n");
			debug("Number of packets sent:  %5d   %5d   %5d",
			...
			debug("Number of packets recv:  %5d   %5d   %5d",
		...
		if (socket_stats) {	
			debug("\nSocket Statistics:\n");
			debug("    Bytes sent:          %lld\n",
			    socket_stats->nbytes_xmit);
			debug("    Bytes received:      %lld\n",
			    socket_stats->nbytes_recv);
